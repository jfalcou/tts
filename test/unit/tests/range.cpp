...
namespace tts
{
  ...
  {
    template<typename U, typename Func> static void run(X const *&src, U *&dst, Func f) noexcept
    {
...
{
  TTS_ULP_RANGE_CHECK(tts::between(0.f, 100.f), float, (std::array<float, 4>), ok_x, ajar_ax, 64);
};

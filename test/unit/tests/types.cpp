...
TTS_CASE_TPL("Check interaction with templates", int, float, char, void *)<typename Type>(
    ::tts::type<Type>)
{
  TTS_TYPE_IS(std::add_const_t<Type>, Type const);
};

TTS_CASE_TPL("Check interaction with pre-made type lists",
             ::tts::arithmetic_types)<typename Type>(::tts::type<Type>)
{
  TTS_TYPE_IS(std::add_const_t<Type>, Type const);
};
...
TTS_CASE_TPL("Check interaction with type list generator", sizes<5>)<typename Type>(tts::type<Type>)
{
  TTS_TYPE_IS(std::add_const_t<Type>, Type const);
};

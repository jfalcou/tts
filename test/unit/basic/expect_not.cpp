...
TTS_CASE("Check that constexpr counter-expectation can be met")
{
  ...
  TTS_CONSTEXPR_EXPECT_NOT(false, REQUIRED);
  TTS_CONSTEXPR_EXPECT_NOT(eq(69, 42), REQUIRED);
  TTS_CONSTEXPR_EXPECT_NOT(neq(42, 42), REQUIRED);
  TTS_CONSTEXPR_EXPECT_NOT(lt(69, 42), REQUIRED);
  TTS_CONSTEXPR_EXPECT_NOT(gt(42, 69), REQUIRED);
  TTS_CONSTEXPR_EXPECT_NOT(lte(69, 42), REQUIRED);
  TTS_CONSTEXPR_EXPECT_NOT(gte(42, 69), REQUIRED);
};

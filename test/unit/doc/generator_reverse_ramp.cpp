...
<typename T>(T const& ramp1, T const& ramp2)
{
  ...
  else
  {
    ...
    auto const last  = ramp1.size() - 1;
    for(std::size_t i = 0; i < ramp1.size(); ++i)
    {
      TTS_EQUAL(ramp1[ i ], static_cast<value_type>(65) + static_cast<value_type>(last - i));
    }
    for(std::size_t i = 0; i < ramp2.size(); ++i)
    {
      TTS_EQUAL(ramp2[ i ], static_cast<value_type>(10) + static_cast<value_type>((last - i) * 2));
    }
  }
};
//! [snippet]

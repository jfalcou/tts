...
Thank you for your interest in TTS. Bug reports, documentation fixes, a missing assertion macro and
whole new kinds of test are all welcome.
...
ask on [our Discord](https://discord.com/channels/692734675726237696/692735300522344468). No
question is too small, and a five minute conversation there often saves a rewrite.
...
merged pull requests. **You are welcome.** A student who wants to see how a test framework reports a
failure is exactly who we wrote this for, and nobody is going to hold a deadline against you.
...
So say so. In the issue, or on
[Discord](https://discord.com/channels/692734675726237696/692735300522344468): "I have two weeks and
I would like to land something real" is a message we answer gladly, and we will point you at
something that fits the time you have. The [good first
issue](https://github.com/jfalcou/tts/labels/good%20first%20issue) label lists work that is
...
```bash
cmake --preset gcc
cmake --build build/gcc --target unit.infrastructure.basic.exe
ctest --preset gcc
```

**Test files are named in `test/CMakeLists.txt`, not globbed**, so a new file is added to the
`FILES` list of the group it belongs to. A new assertion macro also gets a deliberately failing
case, and the expected counts at the end of that file move with it.
...
- **the documentation of what you touched**, with the `tts-doxygen` target still silent, since a
...
**A wrong result or a wrong message.** The fix and its test in the same commit.
...
**An issue rather than a patch.** A wrong result is only actionable with the compiler, its version
and the test that shows it. A Godbolt link that reproduces it replaces all three.

**Work on an open issue.** The [open issues](https://github.com/jfalcou/tts/issues) are all fair
...
[Discord](https://discord.com/channels/692734675726237696/692735300522344468) before you write it: a
design that has been talked through lands, and one that has not tends to need rewriting once it
arrives.
...
cmake --build build/gcc --target tts-doxygen
```

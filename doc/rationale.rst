...
`Goldberg's paper on the subject`_ or to `its simplified version`_.
...
Let us define :math:`\epsilon` -- the machine epsilon [#f1]_ -- as being the smallest
positive floating point number such that :math:`1+\epsilon` is different of :math:`1`
In fact, :math:`1+\epsilon` and :math:`1` only differs by one bit in the least significant digit:
the unit in the last place (ULP). Generally, the ULP between a floating point number
:math:`x` and its immediate successor is :math:`2^{E} \times \epsilon` where :math:`E` is the
exponent of :math:`x`.
...
* The `ulpdist` between :math:`N` and :math:`N\times\epsilon` is equal to :math:`0.5`
...
* The value you compare must be equal by design. In this case, use @ref TTS_EQUAL to clearly state
  your intent. One such case can be for example functions that construct a floating point value bitwise.
...
.. _`Goldberg's paper on the subject`: http://docs.oracle.com/cd/E19957-01/806-3568/ncg_goldberg.html
.. _`its simplified version`: http://floating-point-gui.de/

.. rubric:: Footnotes

.. [#f1] :math:`\epsilon` is equal to  :math:`2^{-52}` for double precision numbers and :math:`2^{-23}` for single precision numbers.
